    ...
    try{
      ...
      if(data.success){
         ...
         if(data.availableCars.length === 0){
           toast('No cars available', { style: { background: '#dbeafe', color: '#1e40af',border: '1px solid #93c5fd', },});
         }
         return null
       }else{
       ...
       <motion.div initial={{opacity:0}} animate={{opacity:1}} transition={{duration: 0.5, delay:0.6}} className="px-6 md:px-16 lg:px-24 xl:px-32 mt-10">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 mt-4 xl:px-20 max-w-7xl mx-auto">
              {filteredCars.map((car, index) => (
                   <motion.div key={index} initial={{y:20, opacity:0}} animate={{y:0, opacity:1}} transition={{duration: 0.4,delay:0.1*index}} >
                      <CarCard car={car} />
                   </motion.div>
              ))}
          </div>
